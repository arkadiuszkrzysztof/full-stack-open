...
const App = () => {
  ...
  const voteOnCurrentAnecdote = (selected) => () => {
    let currentVotes = votes[selected]
    
    setVotes({...votes, [selected]: (currentVotes + 1 || 1)})
    
    if(currentVotes + 1 >= votes[mostVoted])
  ...
  return (
    <div>
      ...
      <Button label="vote" handleClick={voteOnCurrentAnecdote(selected)} />
      <Button label="next anecdote" handleClick={setRandomAnecdote} />
